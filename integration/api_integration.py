...
def fetch_integrated_data(station_id=None, dataset_id=None, time_range=None):
    ...
    if station_id:
        ...
        try:
            ...
            if time_range and len(time_range) == 2:
                ...
                climate_query = climate_query.filter(
            ...
            for data_type in station.available_data_types():
                ...
                agg_data = climate_query.filter(
                    **{f"{data_type}__isnull": False}
                ).aggregate(
            ...
            result["repository_datasets"].append(
                {
                    ...
                    "latest_version": {
                        ...
                        "file_url": latest_version.file_path.url
                    ...
                    "spatial_info": {
                        ...
                        "location": {
                            "latitude": dataset.location.y
                            if dataset.location
                            else None,
                            "longitude": dataset.location.x
                            if dataset.location
                            else None,
                        }
                        if dataset.location
    ...
    if (
        result["station_data"]
        and result["station_data"]["info"]["location"]["latitude"]
    ):
    ...
    if result["station_data"] and result["station_data"]["recent_data"]:
        ...
        for data_type in ["temperature", "humidity", "precipitation", "wind_speed"]:
            if any(
                ...
                for data in result["station_data"]["recent_data"]
            ):
...
def unified_search(query_string):
    ...
    """
    results = {"stations": [], "devices": [], "datasets": [], "climate_data": []}
    ...
    dataset_results = Dataset.objects.filter(
        ...
        "id", "title", "status", "category__name", "created_by__username", "created_at"
    )
...
def process_station_data_to_repository(
    station_id,
    time_range=None,
    creator_id=None,
    title=None,
    ...
    try:
        ...
        if not description:
            description = (
                f"Automatically processed climate data from station {station.name}. "
