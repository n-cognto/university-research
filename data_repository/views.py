...
def my_datasets(request):
    ...
    user_datasets = Dataset.objects.filter(created_by=request.user).order_by(
        "-created_at"
    )
    ...
    context = {"datasets": page_obj, "page_obj": page_obj, "title": "My Datasets"}
...
def download_history(request):
    ...
    downloads = DatasetDownload.objects.filter(user=request.user).order_by(
        "-downloaded_at"
    )
    ...
    context = {"downloads": page_obj, "page_obj": page_obj, "title": "Download History"}
...
class DatasetUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def test_func(self):
        ...
        return (
            ...
            or dataset.access_controls.filter(
...
def add_dataset_to_stack(request, stacked_dataset_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                "repository:stacked_dataset_detail", slug=stacked_dataset.slug
            )
    else:
...
def remove_dataset_from_stack(request, stacked_dataset_id, item_id):
    ...
    item = get_object_or_404(
        StackedDatasetItem, id=item_id, stacked_dataset=stacked_dataset
    )
...
def generate_stacked_dataset(request, stacked_dataset_id):
    ...
    return JsonResponse(
        {"status": "success", "message": "Stacked dataset generation started"}
    )
...
def dataset_version_create(request, dataset_id):
            ...
            try:
                ...
                if uploaded_file:
                    ...
                    if uploaded_file.size > 1024 * 1024 * 100:  # 100 MB limit
                        messages.error(
                            request, "File too large. Maximum file size is 100 MB."
                        )
                        return render(
                            request,
                            "data_repository/version_form.html",
                    ...
                    try:
                        ...
                        if file_ext in [".csv", ".txt"]:
                            ...
                            if csv_metadata.get("time_columns") and csv_metadata.get(
                                "value_columns"
                            ):
                                try:
                                    # Reset file pointer
                                    ...
                                    time_column = (
                                        csv_metadata["time_columns"][0]
                                        if csv_metadata["time_columns"]
                                        else None
                                    )
                                    value_columns = csv_metadata["value_columns"]

                                    # Generate time series preview
                                    time_series_data = csv_to_time_series_json(
                                        uploaded_file, time_column, value_columns
                                    )
                            ...
                            try:
                                ...
                                if isinstance(json_data, dict):
                                    ...
                                    if "metadata" in json_data and isinstance(
                                    ...
                                    # Look for variables
                                    if "variables" in json_data and isinstance(
                                        json_data["variables"], list
                                    ):
                                        metadata["detected_variables"] = json_data[
                                            "variables"
                                        ]

                                    # Look for time series data
                                    if "data" in json_data and isinstance(
                                        json_data["data"], dict
                                    ):
                                ...
                                metadata[
                                    "parse_error"
                                ] = "Could not parse JSON structure from sample"
                            ...
                            try:
                                    ...
                                    with nc.Dataset(tmp_path, "r") as nc_data:
                                        ...
                                        for var_name in nc_data.variables:
                                            if (
                                                var_name.lower() == "time"
                                                or "time"
                                                in nc_data.variables[var_name].ncattrs()
                                            ):
                                        ...
                                        if time_var is not None:
                                            ...
                                            for attr in time_var.ncattrs():
                                                time_attrs[attr] = time_var.getncattr(
                                                    attr
                                                )
                                ...
                                metadata[
                                    "netcdf_note"
                                ] = "NetCDF library not available for metadata extraction"
                            except Exception as e:
                            ...
                            try:
                                ...
                                for col in df.columns:
                                    if (
                                        col.lower().find("time") >= 0
                                        or col.lower().find("date") >= 0
                                    ):
                                ...
                                metadata[
                                    "excel_note"
                                ] = "Pandas library not available for Excel metadata extraction"
                            except Exception as e:
                ...
                success_message = (
                    f"New version {version.version_number} uploaded successfully!"
                )
    ...
    return render(
        request, "data_repository/version_form.html", {"form": form, "dataset": dataset}
    )
...
def category_datasets(request, category_slug):
    ...
    datasets = Dataset.objects.filter(
        Q(category=category) | Q(category__parent=category), status="published"
    )

    return render(
        request,
        "data_repository/category_datasets.html",
...
def dataset_detail(request, dataset_id):
    ...
    try:
        ...
        return render(
            request,
            "data_repository/dataset_not_found.html",
...
def dataset_download(request, dataset_id, version_id=None):
    ...
    if not access or (
        access.access_type == "private" and request.user != dataset.created_by
    ):
...
def dataset_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            dataset.save()
            messages.success(
                request, f'Dataset "{dataset.title}" was successfully created.'
            )
            return redirect("repository:dataset_detail", dataset_id=dataset.id)
        else:
            messages.error(
                request,
                "There was an error creating your dataset. Please check the form and try again.",
            )
    else:
        form = DatasetForm()
...
def version_create(request, dataset_id):
    ...
    if request.user != dataset.created_by:
        messages.error(
            request, "You do not have permission to add versions to this dataset."
        )
        return HttpResponse("Access denied", status=403)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f"Version {version.version_number} was successfully uploaded."
            )
            return redirect("repository:dataset_detail", dataset_id=dataset.id)
        else:
            messages.error(
                request,
                "There was an error uploading your dataset version. Please check the form and try again.",
            )
    else:
        form = DatasetVersionForm()
...
def dataset_timeseries(request, dataset_id, variable):
    ...
    if not current_version or not current_version.has_time_series:
        return JsonResponse(
            {"error": "No time series data available for this dataset"}, status=404
        )
    ...
    if not time_series_data:
        return JsonResponse(
            {"error": f"No data available for variable {variable}"}, status=404
        )
...
def api_dataset_timeseries(request, dataset_id):
    ...
    if not variables:
        return JsonResponse(
            {"error": "No variables available for this dataset"}, status=404
        )
...
def delete_stacked_dataset(request, stacked_dataset_id):
    ...
    if stacked_dataset.created_by != request.user:
        messages.error(
            request, "You do not have permission to delete this stacked dataset."
        )
...
def create_stack_from_selection(request):
    ...
    try:
        ...
        if not name:
            return JsonResponse(
                {"success": False, "error": "Stack name is required"}, status=400
            )
        ...
        for i, dataset_info in enumerate(datasets):
            ...
            item = StackedDatasetItem(
                stacked_dataset=stacked_dataset, dataset=dataset, order=i
            )
        ...
        return JsonResponse(
            {
                ...
                "redirect_url": reverse_lazy(
                    "repository:stacked_dataset_detail",
                    kwargs={"slug": stacked_dataset.slug},
                ),
            }
        )
...
def get_time_series_data(request, version_id):
    ...
    try:
        ...
        file_format = (
            version.metadata.get("format", "").lower() if version.metadata else ""
        )
        ...
        if file_format == "json" or file_path.lower().endswith(".json"):
            ...
            try:
                ...
                return JsonResponse(
                    {"error": f"Error reading JSON data: {str(e)}"}, status=500
                )
            ...
            try:
                ...
                return JsonResponse(
                    {"error": f"Error reading CSV data: {str(e)}"}, status=500
                )
            ...
            try:
                                ...
                                for var_name, variable in nc_data.variables.items():
                                    ...
                                    if var_name != "time" and len(variable.shape) > 0:
                                        ...
                                        if variable.shape[0] == len(time_strings):
                                            ...
                                            values = (
                                                variable[:].data.tolist()
                                                if hasattr(variable[:], "data")
                                                else variable[:].tolist()
                                            )
                                            result[var_name] = {
                                                "times": time_strings,
                                                "values": values,
                ...
                return JsonResponse(
                    {"error": f"Error reading NetCDF data: {str(e)}"}, status=500
                )

        # If no data was extracted, return an empty object with error message
        if not result:
            return JsonResponse(
                {"error": "No time series data found in file"}, status=404
            )
