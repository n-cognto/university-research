...
class EnhancedFileProcessor:
    ...
    def process_file(
        ...
        try:
            ...
            if detected_format == "csv":
                ...
                result["warnings"].append(
                    f"Limited support for format: {detected_format}"
                )
    ...
    def _process_csv_file(
        self, file_obj: UploadedFile, result: Dict, extract_preview: bool
    ):
    ...
    def _process_excel_file(
        ...
        try:
            # Read Excel file
            df = pd.read_excel(
                file_obj, nrows=self.MAX_PREVIEW_ROWS if extract_preview else None
            )
            ...
            if len(xls.sheet_names) > 1:
                ...
                result["warnings"].append(
                    f"Multiple sheets detected: {xls.sheet_names}"
                )
    ...
    def _validate_csv_structure(self, metadata: Dict, result: Dict):
        ...
        empty_cols = [
            col
            for col, dtype in metadata.get("data_types", {}).items()
            if dtype == "empty"
        ]
        if empty_cols:
    ...
    def _load_validation_rules(self) -> Dict:
        ...
        return {
            "csv": {"max_columns": 1000, "max_rows": 1000000, "required_headers": True},
            "json": {"max_depth": 10, "max_keys": 10000},
        }


# Utility functions
def validate_file_upload(
    file_obj: UploadedFile, allowed_formats: List[str] = None, max_size: int = None
) -> Tuple[bool, List[str]]:
    ...
    if allowed_formats:
        ...
        if ext not in allowed_exts:
            errors.append(
                f"Unsupported file format. Allowed: {', '.join(allowed_exts)}"
            )
