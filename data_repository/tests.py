...
class JSONImportTests(TestCase):
    ...
    def test_json_to_csv_object_timeseries(self):
        ...
        self.assertIn(
            "timestamp,humidity,temp", csv_output
        )  # or timestamp,temp,humidity
        self.assertIn(
            "2023-01-01T00:00:00Z,60,15", csv_output
        )  # or 2023-01-01T00:00:00Z,15,60
        self.assertIn(
            "2023-01-01T01:00:00Z,62,16", csv_output
        )  # or 2023-01-01T01:00:00Z,16,62
...
class CSVImportTests(TestCase):
    def test_extract_csv_metadata(self):
        ...
        )  # Includes header row for this counter
    ...
    def test_csv_to_time_series_json_iso_datetime(self):
        ...
        csv_file = io.StringIO(csv_data)
        json_output_str = csv_to_time_series_json(
            csv_file, time_column="datetime", value_columns=["temp"]
        )
        json_output = json.loads(json_output_str)

        self.assertIsInstance(
            json_output["data"]["temp"]["times"][0], int
        )  # Check if timestamp is converted to ms
    ...
    def test_csv_import_error_exception(self):
        with self.assertRaisesRegex(CSVImportError, "Test CSV error"):
            raise CSVImportError(
                "Test CSV error", row=["a", "b"], line_num=1, field="test_field"
            )
