"""
Script to add field data collection models to the database.
This script should be run after the models have been added to the codebase.

Usage:
    python manage.py shell < maps/add_field_data_collection.py
"""
...
def apply_migrations():
    ...
    for i, sql in enumerate(SQL_STATEMENTS):
        ...
        try:
            cursor.execute(sql)
            connection.commit()
            print(f"✅ {statement_name} executed successfully.")
            success_count += 1
        except (IntegrityError, ProgrammingError) as e:
            error_message = str(e)
            if "already exists" in error_message:
                print(f"⚠️ {statement_name} skipped: {error_message}")
            else:
                print(f"❌ Error executing {statement_name}: {error_message}")
                error_count += 1
