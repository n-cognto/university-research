...
DEFAULT_EMAIL_FROM = getattr(
    settings, "DEFAULT_FROM_EMAIL", "noreply@weatherstations.example.com"
)
...
def batch_fetch_environmental_data(
    api_key: str = None, batch_size: int = 10, delay: int = 1
) -> Dict[str, int]:
    """
...
def fetch_station_data(station: WeatherStation, api_key: str = None) -> bool:
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error(
                f"Failed to fetch data for station {station.name}. Status code: {response.status_code}"
            )
            return False
    ...
    for i in range(0, stations.count(), batch_size):
        ...
        for station in batch:
            try:
                ...
                logger.error(
                    f"Error in batch processing for station {station.name}: {str(e)}"
                )
                total_results["error"] += 1
...
def check_alert_thresholds(
    alert_config: Dict = None, send_notifications: bool = True
) -> Dict[str, int]:
    """
...
def create_alert_from_detection(
    station: WeatherStation, alert_data: Dict
) -> WeatherAlert:
    """
...
def _evaluate_parameter_alerts(parameter, value, thresholds):
    ...
    if "danger_high" in thresholds and value >= thresholds["danger_high"]:
        alerts.append(
            _create_alert_dict(
                parameter, value, thresholds["danger_high"], "danger", "Extreme High"
            )
        )
    elif "warning_high" in thresholds and value >= thresholds["warning_high"]:
        alerts.append(
            _create_alert_dict(
                parameter, value, thresholds["warning_high"], "warning", "High"
            )
        )
    elif "danger_low" in thresholds and value <= thresholds["danger_low"]:
        alerts.append(
            _create_alert_dict(
                parameter, value, thresholds["danger_low"], "danger", "Extreme Low"
            )
        )
    elif "warning_low" in thresholds and value <= thresholds["warning_low"]:
        alerts.append(
            _create_alert_dict(
                parameter, value, thresholds["warning_low"], "warning", "Low"
            )
        )
    return alerts
...
def detect_anomalies(
    station_id: int = None, days: int = 30, std_dev_threshold: float = 2.0
) -> List[Dict]:
    """
...
def process_csv_data(
    file_path: str, import_type: str, update_existing: bool = True
) -> Dict[str, Any]:
    """
    ...
    processor = (
        process_stations_csv if import_type == "stations" else process_climate_data_csv
    )
...
def _process_csv(csv_file, update_existing, row_processor, data_type, required_fields):
    result = {"success": 0, "error": 0, "errors": [], "type": data_type}

    try:
        decoded_file = (
            csv_file.read().decode("utf-8")
            if hasattr(csv_file, "read") and not isinstance(csv_file.read(), str)
            else csv_file.read()
        )
...
def _validate_csv_rows(reader, header, import_type):
    ...
    for row in reader:
        ...
        if row_count <= 5 and len(row) != len(header):
            warnings.append(
                f"Row {row_count} has {len(row)} fields, expected {len(header)}"
            )
...
def _process_station_row(row, result, update_existing, required_fields):
    ...
    if update_existing:
        WeatherStation.objects.update_or_create(name=row["name"], defaults=defaults)
    else:
...
def _process_climate_data_row(row, result, update_existing, required_fields):
    ...
    if update_existing:
        ClimateData.objects.update_or_create(
            station=station, timestamp=timestamp, defaults=climate_data
        )
    else:
        if not ClimateData.objects.filter(
            ...
            result["errors"].append(
                f"Data already exists for {station.name} at {timestamp}"
            )
            return
...
def export_data_to_csv(
    ...
    filename = (
        f"{station.name.replace(' ', '_')}_{start_date.strftime('%Y%m%d')}_to_{end_date.strftime('%Y%m%d')}.csv"
        if station_id
        else f"all_stations_{start_date.strftime('%Y%m%d')}_to_{end_date.strftime('%Y%m%d')}.csv"
    )
    csv_file.seek(0)
...
def find_nearest_stations(
    ...
    stations = (
        WeatherStation.objects.filter(location__distance_lte=(point, D(km=radius_km)))
...
def setup_alert_checks(check_interval=300):
    ...
    try:
        ...
        logger.warning(
            "APScheduler not installed. Automated alert checks will not run."
        )
...
def create_secure_export_file(export, queryset, format_type):
    ...
    stations_str = (
        "all_stations"
        if not export.stations.exists()
        else f"{export.stations.count()}_stations"
    )
...
def _process_station_row_enhanced(
    row, line_num, progress, update_existing=True, required_fields=None
):
    """
        ...
        if "country" in row and row["country"]:
            try:
                ...
                try:
                    ...
                    progress.warning(
                        f"Country not found: {row['country']}",
                        row=row,
                        line_num=line_num,
                        field="country",
                    )
...
def _process_climate_data_row_enhanced(
    row, line_num, progress, update_existing=True, required_fields=None
):
    """
    ...
    for file_path in csv_files:
        try:
            ...
            if file_type == "stations":
                ...
                if file_result.get("errors"):
                    results["errors"].extend(
                        [
                            f"{file_name}: {e.get('message', 'Unknown error')}"
            ...
            processed_path = os.path.join(
                temp_dir, f"{file_name}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
            )
            try:
                import shutil
...
def determine_file_type(file_path):
    ...
    if any(
        keyword in filename
        for keyword in ["climate", "weather", "data", "readings", "measurements"]
    ):
        return "climate_data"
