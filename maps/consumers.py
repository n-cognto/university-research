...
class FieldDataConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        await self.send(
            text_data=json.dumps({"type": "initial_data", "devices": devices})
        )
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            if message_type == "heartbeat_ack":
                ...
                if (
                    hasattr(self, "last_received_seq")
                    and self.last_received_seq is not None
                ):
                    missed_messages = await self.get_missed_messages(
                        self.last_received_seq
                    )
                    if missed_messages:
                        await self.send(
                            text_data=json.dumps(
                                {"type": "missed_messages", "messages": missed_messages}
                            )
                        )
                ...
                if devices:
                    for device_id in devices:
                        await self.channel_layer.group_add(
                            f"field_data_{device_id}", self.channel_name
                        )

                    await self.send(
                        text_data=json.dumps(
                            {"type": "subscription_success", "devices": devices}
                        )
                    )
                ...
                if devices:
                    ...
                    await self.send(
                        text_data=json.dumps(
                            {"type": "unsubscription_success", "devices": devices}
                        )
                    )
    ...
    def get_device_config(self, device_id):
        ...
        try:
            ...
            config = {
                ...
                if hasattr(device, "sampling_rate")
                else 60,
                "reporting_interval": device.reporting_interval
                if hasattr(device, "reporting_interval")
                else 300,
                "power_save_mode": device.power_save_mode
                if hasattr(device, "power_save_mode")
                else "normal",
                "alert_thresholds": device.alert_thresholds
                if hasattr(device, "alert_thresholds")
                else {},
                "sensors_enabled": device.sensors_enabled
                if hasattr(device, "sensors_enabled")
                else [],
    ...
    def _get_device_status(self, device_id):
        ...
        latest_record = (
            FieldDataRecord.objects.filter(device_id=device_id)
            .order_by("-timestamp")
            .first()
        )
    ...
    def _get_device(self, device_id):
        ...
        try:
            device = FieldDevice.objects.get(id=device_id)
            latest_record = (
                FieldDataRecord.objects.filter(device=device)
                .order_by("-timestamp")
                .first()
            )
...
class BatchDataConsumer(AsyncWebsocketConsumer):
            ...
            text_data=json.dumps(
                {
                    ...
                    "max_payload_size": getattr(
                        settings, "WEBSOCKET_MAX_PAYLOAD_SIZE", 10 * 1024 * 1024
                    ),  # 10MB default
                    "protocol_versions": [
                        "1.0",
                        "1.1",
                        "2.0",
                    ],  # Include protocol versions supported
                    "server_time": timezone.now().isoformat(),
                }
            )
        )
        ...
        try:
            ...
            if bytes_data:
                ...
                if message_type == "protocol_selection":
                    ...
                    await self.send(
                        text_data=json.dumps(
                            {"type": "protocol_accepted", "version": protocol_version}
                        )
                    )
                    ...
                    self.current_batch_id = (
                        f"{self.device_id}_{timezone.now().timestamp()}"
                    )
                    ...
                    if self.chunks_received == self.total_chunks:
                        result = await self.process_optimized_batch(
                            self.device_id, self.chunk_data
                        )
    ...
    async def process_binary_data(self, binary_data):
        ...
        try:
            ...
            if message_type == 1:  # Batch data in MessagePack format
                ...
                result = await self.process_optimized_batch(
                    self.device_id, unpacked_data.get("records", [])
                )
                ...
                await self.send(
                    bytes_data=struct.pack(header_format, 101) + msgpack.packb(response)
                )
                ...
                # Send response as binary MessagePack
                response = {"status": "success", "records_created": result["created"]}

                await self.send(
                    bytes_data=struct.pack(header_format, 102) + msgpack.packb(response)
                )
                ...
                await self.send(
                    bytes_data=struct.pack(header_format, 255) + msgpack.packb(response)
                )
            ...
            try:
                ...
                await self.send(
                    bytes_data=struct.pack("!I", 255) + msgpack.packb(response)
                )
            except:
            ...
            with transaction.atomic():
                ...
                if connection.vendor == "postgresql":
                    ...
                    with connection.cursor() as cursor:
                        ...
                        cursor.copy_from(
                            file=data_file, table=table_name, columns=columns
                        )
                else:
    ...
    @database_sync_to_async
    def process_stacked_timeseries(
        self, station_id, start_time, end_time, interval, metrics
    ):
        """
            ...
            with transaction.atomic():
                ...
                for i, timestamp in enumerate(date_range):
                    ...
                    for metric_name, values in metrics.items():
                        if metric_name not in ("latitude", "longitude") and isinstance(
                            values, list
                        ):
                    ...
                    record = FieldDataRecord(
                        ...
                        timestamp=timestamp,
                        latitude=lat[i]
                        if isinstance(lat, list) and i < len(lat)
                        else None,
                        longitude=lon[i]
                        if isinstance(lon, list) and i < len(lon)
                        else None,
                        data=data,
                    )
                    records_to_create.append(record)
