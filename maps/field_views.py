...
def field_data_upload(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not title or not data_file or not data_format:
                messages.error(
                    request, "Please provide a title, data format, and upload a file."
                )
            ...
            messages.success(
                request, f'File "{title}" uploaded successfully and is being processed.'
            )
    ...
    uploads = FieldDataUpload.objects.filter(created_by=request.user).order_by(
        "-upload_date"
    )[:10]

    return render(
        request,
        "maps/field_data_upload.html",
        {"device_types": device_types, "uploads": uploads},
    )
...
def field_data_upload_detail(request, upload_id):
    ...
    return render(
        request,
        "maps/field_data_upload_detail.html",
        {"upload": upload, "data_preview": data_preview},
    )
...
def field_data_visualize(request, upload_id):
    ...
    if upload.status != "completed":
        messages.error(
            request,
            "This upload is not yet processed. Please wait until processing is complete.",
        )
        return redirect("maps:field_data_upload_detail", upload_id=upload.id)
    ...
    devices = (
        FieldDevice.objects.filter(device_type=upload.device_type)
        if upload.device_type
        else FieldDevice.objects.all()
    )

    return render(
        request,
        "maps/field_data_visualize.html",
        {
            "upload": upload,
        ...
        if upload.data_format == "csv":
            ...
            if isinstance(data, list) and len(data) > 0:
                ...
                if len(data["data"]) > 0:
                    ...
                    rows = [
                        [item.get(h, "") for h in headers] for item in data["data"][:10]
                    ]
                    return {"headers": headers, "rows": rows}
            ...
            return {
                "headers": ["Key", "Value"],
                "rows": [[k, str(v)] for k, v in list(data.items())[:10]],
            }
