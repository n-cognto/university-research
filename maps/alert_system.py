...
def check_for_alerts(device, sensor_data):
    ...
    for alert in alerts:
        ...
        logger.log(
            log_level,
            f"Alert for {device.name} ({device.device_id}): {alert['message']}",
        )
...
def send_alert_notifications(alert):
    ...
    try:
        subject = (
            f"CRITICAL ALERT: {alert.device.name} - {alert.get_alert_type_display()}"
        )
        message = f"""
...
def check_device_connection_status():
    ...
    for device in disconnected_devices:
        ...
        logger.warning(
            f"Connection lost alert created for {device.name} ({device.device_id})"
        )
