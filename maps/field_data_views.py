        ...
        for record in device_records:
            ...
            if data.get("temperature") is not None:
                temperature_data.append(
                    {"timestamp": timestamp, "value": data.get("temperature")}
                )

            if data.get("humidity") is not None:
                humidity_data.append(
                    {"timestamp": timestamp, "value": data.get("humidity")}
                )

            if data.get("battery_voltage") is not None:
                battery_data.append(
                    {"timestamp": timestamp, "value": data.get("battery_voltage")}
                )

            if data.get("signal_strength") is not None:
                signal_data.append(
                    {"timestamp": timestamp, "value": data.get("signal_strength")}
                )
