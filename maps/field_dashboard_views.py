...
def field_data_dashboard(request):
    ...
    # Get recent alerts
    recent_alerts = (
        Alert.objects.filter(timestamp__gte=timezone.now() - timedelta(hours=24))
...
def device_data_api(request, device_id):
    ...
    try:
        ...
        records = FieldDataRecord.objects.filter(device=device).order_by("-timestamp")[
            :100
...
def device_alerts_api(request, device_id):
    ...
    try:
        ...
        alerts = Alert.objects.filter(
            device=device, timestamp__gte=timezone.now() - timedelta(days=7)
        ).order_by("-timestamp")[:10]
...
def device_reset_api(request, device_id):
    ...
    try:
        ...
        return Response(
            {"success": True, "message": f"Device {device.name} has been reset"}
        )
        return Response({"error": "Device not found"}, status=404)
    except Exception as e:
