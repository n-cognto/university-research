...
def device_data_upload(request):
    ...
    try:
        ...
        if not is_valid:
            return JsonResponse(
                {"error": "Validation failed", "errors": errors, "received_data": data},
                status=400,
            )
        ...
        if not device:
            ...
            if not device_type:
                return JsonResponse(
                    {"error": "No device type configured", "device_id": device_id},
                    status=500,
                )
        ...
        try:
            ...
            os.makedirs(
                os.path.dirname(f"{settings.MEDIA_ROOT}/{filename}"), exist_ok=True
            )
