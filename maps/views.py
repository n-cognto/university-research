    ...
    return JsonResponse(
        {
            ...
            "debug_info": {
                "fields_available": [
                    field.name for field in WeatherStation._meta.fields
                ],
                "data_types": list(
                    WeatherDataType.objects.values_list("name", flat=True)
                ),
                "data_types": list(
                    WeatherDataType.objects.values_list("name", flat=True)
                ),
                ...
                return Response(
                    {
                        ...
                        "device_id": device_id,
                        "available_device_types": list(
                            DeviceType.objects.values_list("name", flat=True)
                        ),
                    },
...
class MapView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        recent_data = (
            ClimateData.objects.select_related("station")
        ...
        active_alerts = (
            WeatherAlert.objects.select_related("station", "country", "data_type")
...
class WeatherStationViewSet(viewsets.ModelViewSet):
        ...
        for station in queryset:
            feature = {
                ...
                "properties": {
                    ...
                    "date_installed": station.date_installed.isoformat()
    ...
    def nearby(self, request):
        ...
        stations = (
            WeatherStation.objects.filter(location__distance_lte=(point, D(km=radius)))
    ...
    def data(self, request, pk=None):
        ...
        query = ClimateData.objects.filter(
            station=station, timestamp__gte=start_date
        ).order_by("-timestamp")
    ...
    def export(self, request, pk=None):
        ...
        if format_type not in ["csv", "json", "geojson"]:
            return Response(
                {"error": "Format must be one of: csv, json, geojson"}, status=400
            )
        ...
        if format_type == "csv":
            ...
            response[
                "Content-Disposition"
            ] = f'attachment; filename="{station.name}_{timezone.now().strftime("%Y%m%d")}.csv"'
            ...
            response[
                "Content-Disposition"
            ] = f'attachment; filename="{station.name}_{timezone.now().strftime("%Y%m%d")}.json"'
            return response
            ...
            response[
                "Content-Disposition"
            ] = f'attachment; filename="{station.name}_{timezone.now().strftime("%Y%m%d")}.geojson"'
            return response
    ...
    def statistics(self, request, pk=None):
        ...
        return Response(
            {"station": station.name, "period": f"Last {days} days", "stats": stats}
        )
...
class WeatherAlertViewSet(viewsets.ModelViewSet):
    queryset = WeatherAlert.objects.all().select_related(
...
def climate_data_station(request, station_id=None):
    ...
    try:
        ...
        return Response(
            {"error": f"Weather station with ID {station_id} not found"}, status=404
        )
    except Exception as e:
    ...
    try:
        ...
        for station in active_stations:
            try:
                ...
                latest_reading = (
                    ClimateData.objects.filter(station=station)
                    .order_by("-timestamp")
                    .values(
                        "id",
                        "timestamp",
                ...
                if latest_reading:
                    ...
                    for data_type in station.available_data_types():
                        if (
                            data_type in latest_reading
                            and latest_reading[data_type] is not None
                        ):
                            data["data"][data_type] = latest_reading[data_type]

                    data["is_recent"] = latest_reading["timestamp"] > (
        ...
        return Response(
            {
                ...
                "stats": {
                    ...
                    "active_stations": active_stations.count(),
                    "stations_with_recent_data": len(
                        [d for d in recent_data if d["is_recent"]]
                    ),
                    "active_alerts": active_alerts.count(),
                    "available_data_types": list(
                        WeatherDataType.objects.values("name", "display_name", "unit")
                    ),
                },
            }
        )
...
class CSVUploadView(FormView):
    ...
    def form_valid(self, form):
        try:
            ...
            # Enhanced logging
            logging.info(
                f"Processing CSV import of type: {import_type}, file: {csv_file.name}, size: {csv_file.size} bytes, mode: {processing_mode}"
            )
            ...
            # Enhanced logging
            logging.info(
                f"CSV import result: {result['success']} success, {result['error']} errors"
            )
    ...
    def _process_climate_data_to_stack(self, row, line_num, progress):
        ...
        try:
            ...
            raise CSVImportError(
                f"Error finding station: {str(e)}", row=row, line_num=line_num
            )
        ...
        try:
            ...
            raise CSVImportError(
                f"Error adding data to stack: {str(e)}", row=row, line_num=line_num
            )
    ...
    def _process_weather_data_type_row(self, row, line_num, progress):
        ...
        if "min_value" in row and row["min_value"]:
            try:
                ...
                progress.warning(
                    e.message, row=row, line_num=line_num, field="min_value"
                )
        ...
        if "max_value" in row and row["max_value"]:
            try:
                ...
                progress.warning(
                    e.message, row=row, line_num=line_num, field="max_value"
                )
        ...
        try:
            ...
            raise CSVImportError(
                f"Error creating weather data type: {str(e)}",
                row=row,
                line_num=line_num,
            )
    ...
    def _process_country_row(self, row, line_num, progress):
        ...
        if not (len(code) == 2 or len(code) == 3):
            progress.warning(
                f"Country code '{code}' should be 2 or 3 letters",
                row=row,
                line_num=line_num,
                field="code",
            )
        ...
        try:
            ...
            raise CSVImportError(
                f"Error creating country: {str(e)}", row=row, line_num=line_num
            )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not is_safe_path("/media", drive_path):
                messages.error(
                    request,
                    "Invalid drive path. Path must be within the media directory.",
                )
            ...
            if not os.path.exists(drive_path):
                messages.error(
                    request, f"The specified path does not exist: {drive_path}"
                )
                return render(request, "maps/flash_drive_import.html", {"form": form})
            ...
            try:
                ...
                messages.error(
                    request, f"Permission denied when trying to access: {drive_path}"
                )
                return render(request, "maps/flash_drive_import.html", {"form": form})
            except Exception as e:
            ...
            if not csv_files:
                messages.error(
                    request, "No CSV files found in the specified directory."
                )
                return render(request, "maps/flash_drive_import.html", {"form": form})

            # Process all CSV files
            results = {"success_total": 0, "error_total": 0, "file_results": []}
...
def setup_automated_imports(flash_drive_path=None, import_interval=3600):
    ...
    try:
            ...
            try:
                ...
                if os.path.exists(flash_drive_path):
                    ...
                    try:
                        csv_files = [
                            f
                            for f in os.listdir(flash_drive_path)
                            if f.endswith(".csv")
                        ]
                    except PermissionError:
                        logging.error(
                            f"Permission denied when accessing {flash_drive_path}"
                        )
                        return
                    except Exception as e:
                        logging.error(
                            f"Error accessing directory {flash_drive_path}: {str(e)}"
                        )
                        return
                    ...
                    process_files(
                        flash_drive_path, "datatype_", "weather_data_types", csv_files
                    )
                    process_files(flash_drive_path, "country_", "countries", csv_files)
                    process_files(
                        flash_drive_path, "datatype_", "weather_data_types", csv_files
                    ...
                    process_files(
                        flash_drive_path, "climate_", "climate_data", csv_files
                    )
                else:
                    logging.warning(
                        f"Flash drive path {flash_drive_path} does not exist"
                    )
            except Exception as e:
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            logging.error(
                                f"Cannot decode file {file_name}: Encoding issues"
                            )
                            continue
                    ...
                    # Log the result
                    logging.info(
                        f"Imported {file_name}: {result['success']} success, {result['error']} errors"
                    )
        ...
        scheduler.start()
        logging.info(
            f"Scheduled automated CSV imports from {flash_drive_path} every {import_interval} seconds"
        )
...
def station_statistics_view(request, station_id):
    ...
    stats_30 = ClimateData.objects.filter(
        station=station, timestamp__gte=days_30
    ).aggregate(
        avg_temp=Avg("temperature"),
        max_temp=Max("temperature"),
        min_temp=Min("temperature"),
    ...
    stats_90 = ClimateData.objects.filter(
        station=station, timestamp__gte=days_90
    ).aggregate(
        avg_temp=Avg("temperature"),
        max_temp=Max("temperature"),
        min_temp=Min("temperature"),
    ...
    stats_365 = ClimateData.objects.filter(
        station=station, timestamp__gte=days_365
    ).aggregate(
        avg_temp=Avg("temperature"), total_precipitation=Sum("precipitation", default=0)
    )

    # Get recent climate data points for this station
    recent_data = ClimateData.objects.filter(station=station).order_by("-timestamp")[
        :100
    ]
    ...
    for field in data_type_fields:
        if (
            ...
            available_data_types.append(
                {"field": field, "display_name": display_name, "unit": unit}
            )
...
def station_data_export_view(request, station_id):
    ...
    earliest_record = (
        ClimateData.objects.filter(station=station).order_by("timestamp").first()
    )
    latest_record = (
        ClimateData.objects.filter(station=station).order_by("-timestamp").first()
    )
...
def download_station_data(request, station_id):
    ...
    if date_from_str:
        try:
            ...
            messages.warning(
                request, f"Invalid from date format: {date_from_str}. Using default."
            )
    ...
    if date_to_str:
        try:
            ...
            messages.warning(
                request, f"Invalid to date format: {date_to_str}. Using default."
            )
    ...
    def recent(self, request):
        ...
        subquery = (
            ClimateData.objects.filter(
                station=OuterRef("station"), timestamp__gte=since
            )
            .order_by("-timestamp")
            .values("id")[:1]
        )

        query = ClimateData.objects.filter(id__in=Subquery(subquery)).select_related(
            "station"
        )
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            if success:
                ...
                return Response(
                    {"success": False, "message": "Stack is full"}, status=400
                )
        else:
            return Response(serializer.errors, status=400)
    except Exception as e:
...
def climate_data_recent(request):
    ...
    for station in stations:
        latest = (
            ClimateData.objects.filter(station=station, timestamp__gte=since)
            .order_by("-timestamp")
            .first()
        )
...
def secure_export_download(request, export_id, signature):
    ...
    try:
        ...
        response[
            "Content-Disposition"
        ] = f'attachment; filename="{os.path.basename(export.file.name)}"'
...
def station_graph_data(request, station_id):
    ...
    queryset = (
        ClimateData.objects.filter(
            station=station, timestamp__gte=start_date, timestamp__lte=now
        )
        .annotate(date=date_trunc("timestamp"))
        .values("date")
    )
    ...
    for entry in data:
        ...
        formatted_data["timestamps"].append(
            {"display": date_str, "timestamp": timestamp}
        )
...
def station_data_view(request):
    ...
    context = {"title": "Weather Station Data", "station_id": station_id}
...
def station_statistics_view(request, station_id):
    ...
    try:
        ...
        recent_data = ClimateData.objects.filter(
            station=station, timestamp__gte=timezone.now() - timedelta(days=30)
        ).order_by("-timestamp")
        ...
        stats = {
            ...
            "precipitation": {
                "avg": recent_data.aggregate(Avg("precipitation"))[
                    "precipitation__avg"
                ],
                "max": recent_data.aggregate(Max("precipitation"))[
                    "precipitation__max"
                ],
                "min": recent_data.aggregate(Min("precipitation"))[
                    "precipitation__min"
                ],
                "total": recent_data.aggregate(Sum("precipitation"))[
                    "precipitation__sum"
                ],
            },
            "humidity": {
        ...
        station_stacks.append(
            {
                ...
                "percentage": (stack_size / station.max_stack_size * 100)
                if station.max_stack_size > 0
                else 0,
            }
        )
...
def process_station_stack(request, station_id):
    ...
    if request.method == "POST":
        ...
        if records_processed > 0:
            messages.success(
                request,
                f"Successfully processed {records_processed} readings from {station.name}.",
            )
        else:
