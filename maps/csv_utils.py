...
class CSVImportProgress:
    ...
    def error(self, error_message, row=None, line_num=None, field=None):
        ...
        error_detail = {"message": error_message, "line": line_num, "field": field}
        if row:
    ...
    def warning(self, warning_message, row=None, line_num=None, field=None):
        ...
        warning_detail = {"message": warning_message, "line": line_num, "field": field}
        if row:
            # Limit row data to avoid overwhelming memory
...
def read_csv_file(csv_file: UploadedFile) -> Tuple[List[Dict[str, str]], List[str]]:
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "File encoding detection failed, using latin-1 with replacement"
            )
        except Exception as e:
...
def process_csv_in_batches(
    ...
    for i, row in enumerate(reader, start=1):
        ...
        if len(batch) >= batch_size:
            ...
            logger.info(
                f"Processed batch {batch_num}: {progress.success_count} success, {progress.error_count} errors"
            )
...
def _process_batch(
    batch: List[Tuple[Dict[str, str], int]],
        ...
        with transaction.atomic():
            for row, line_num in batch:
                try:
                    ...
                    progress.error(
                        e.message, row=e.row, line_num=e.line_num, field=e.field
                    )
                except Exception as e:
                    # Unexpected errors
...
def validate_csv_headers(
    fieldnames: List[str], required_fields: List[str]
) -> List[str]:
    """
    ...
    for field in required_check:
        ...
        if (
            field_lower not in clean_fieldnames
            and field_no_underscore not in clean_fieldnames
            and field_spaced not in clean_fieldnames
        ):
...
def parse_numeric(
    ...
    if value is None or (isinstance(value, str) and not value.strip()):
        if allow_none:
            return None
        raise CSVImportError(
            f"Missing required numeric value", field=field_name, line_num=line_num
        )
    ...
    if isinstance(value, (int, float)):
        ...
        try:
            ...
            raise CSVImportError(
                f"Invalid numeric value '{value}'", field=field_name, line_num=line_num
            )
...
def parse_date(
    ...
    if value is None or (isinstance(value, str) and not value.strip()):
        if allow_none:
            return None
        raise CSVImportError(
            f"Missing required date value", field=field_name, line_num=line_num
        )
    ...
    raise CSVImportError(
        f"Invalid date format '{value}'", field=field_name, line_num=line_num
    )


def parse_boolean(
    value: Union[str, bool, int], field_name: str, line_num: int, default: bool = None
) -> Optional[bool]:
    """
    ...
    if value is None or (isinstance(value, str) and not value.strip()):
        if default is not None:
            return default
        raise CSVImportError(
            f"Missing boolean value", field=field_name, line_num=line_num
        )
    ...
    if value_lower in true_values:
        ...
        if default is not None:
            return default
        raise CSVImportError(
            f"Invalid boolean value '{value}'", field=field_name, line_num=line_num
        )
...
def find_station_by_identifier(identifier: str, field_name: str, line_num: int):
    ...
    if not identifier:
        raise CSVImportError(
            "Missing station identifier", field=field_name, line_num=line_num
        )
    ...
    if not station:
        raise CSVImportError(
            f"Station not found: {identifier}", field=field_name, line_num=line_num
        )
...
def validate_column_values(
    ...
    for field, criteria in validations.items():
        ...
        if field not in row:
            if required:
                progress.error(
                    f"Missing required field: {field}", row=row, line_num=line_num
                )
            continue
        ...
        try:
            if field_type == "numeric":
                ...
                result[field] = parse_numeric(
                    value, field, line_num, min_value=min_val, max_value=max_val
                )
