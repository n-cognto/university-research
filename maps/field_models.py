...
class Alert(models.Model):
    ...
    device = models.ForeignKey(
        "FieldDevice", on_delete=models.CASCADE, related_name="alerts"
    )
...
class DeviceType(models.Model):
    ...
    communication_protocol = models.CharField(
        max_length=20, choices=PROTOCOL_CHOICES, default="wifi"
    )
    power_source = models.CharField(
        max_length=20, choices=POWER_CHOICES, default="battery"
    )
...
class FieldDevice(models.Model):
    ...
    transmission_interval = models.PositiveIntegerField(
        default=60, help_text="Data transmission interval in minutes"
    )
...
class DeviceCalibration(models.Model):
    ...
    device = models.ForeignKey(
        FieldDevice, on_delete=models.CASCADE, related_name="calibrations"
    )
...
class FieldDataRecord(models.Model):
    ...
    upload = models.ForeignKey(
        "FieldDataUpload", on_delete=models.CASCADE, related_name="records"
    )
    device = models.ForeignKey(
        FieldDevice, on_delete=models.CASCADE, related_name="data_records"
    )
...
class FieldDataUpload(models.Model):
    ...
    data_format = models.CharField(
        max_length=10, choices=DATA_FORMAT_CHOICES, default="csv"
    )
    device_type = models.ForeignKey(
        DeviceType,
        on_delete=models.SET_NULL,
    ...
    created_by = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True,
        related_name="field_data_uploads",
    )
...
class FieldDataUploadManual(models.Model):
    ...
    uploader = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="field_uploads"
    )
    weather_station = models.ForeignKey(
        WeatherStation, on_delete=models.CASCADE, related_name="field_uploads"
    )
    device = models.ForeignKey(
        FieldDevice,
        on_delete=models.SET_NULL,
    ...
    def _create_climate_data(self):
        ...
        if (
            ...
            or self.precipitation is not None
        ):
