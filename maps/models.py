...
class WeatherStation(models.Model):
    ...
    location = models.PointField(
        srid=4326, geography=True, spatial_index=True
    )  # Add spatial_index=True for better performance
    altitude = models.FloatField(
        help_text="Altitude in meters above sea level", null=True, blank=True
    )
    country = models.ForeignKey(
        Country, on_delete=models.SET_NULL, null=True, related_name="stations"
    )
    region = models.CharField(
        max_length=100,
        blank=True,
        null=True,
        help_text="Administrative region within country",
    )
    ...
    try:
        ...
        data_stack = models.TextField(
            blank=True,
            null=True,
            help_text="JSON serialized stack of pending data readings",
        )

    last_data_feed = models.DateTimeField(
        null=True, blank=True, help_text="Timestamp of last data feed"
    )
    max_stack_size = models.PositiveIntegerField(
        default=1000, help_text="Maximum number of items in the data stack"
    )
    auto_process = models.BooleanField(
        default=False,
        help_text="Whether to automatically process data when stack reaches threshold",
    )
    process_threshold = models.PositiveIntegerField(
        default=100, help_text="Threshold for auto-processing stack data"
    )

    # Relationship to Dataset model
    datasets = models.ManyToManyField(
        Dataset, blank=True, related_name="weather_stations"
    )
    ...
    def push_data(self, data_dict):
        ...
        if not isinstance(data_dict, dict):
            logger.error(
                f"Invalid data format for station {self.id}: Expected dict, got {type(data_dict)}"
            )
            return False
        ...
        if self.auto_process and len(stack_data) >= self.process_threshold:
            logger.info(
                f"Auto-processing {len(stack_data)} readings for station {self.id}"
            )
    ...
    def process_data_stack(self):
        ...
        try:
            with transaction.atomic():
                ...
                for data_reading in stack_data:
                    ...
                    climate_data = ClimateData(
                        ...
                        **{
                            k: v
                            for k, v in data_reading.items()
                            if hasattr(ClimateData, k)
                        },
                    )
            ...
            logger.info(
                f"Successfully processed {count} readings for station {self.id}"
            )
            return count
            ...
            logger.error(
                f"Error processing data stack for station {self.id}: {str(e)}",
                exc_info=True,
            )
            return 0
    ...
    def export_to_repository(self, title, description, category_id, creator_id):
        ...
        try:
            ...
            if climate_data.exists():
                ...
                with open(temp_path, "rb") as file_obj:
                    version = DatasetVersion.objects.create(
                        ...
                        file_path=File(
                            file_obj, name=f"{self.name.replace(' ', '_')}_data.csv"
                        ),
                        is_current=True,
                        metadata={
                            "row_count": climate_data.count(),
                            "time_series": True,
                            "time_start": climate_data.earliest(
                                "timestamp"
                            ).timestamp.isoformat()
                            if climate_data.exists()
                            else None,
                            "time_end": climate_data.latest(
                                "timestamp"
                            ).timestamp.isoformat()
                            if climate_data.exists()
                            else None,
                            ...
                            "variables": [
                                dt
                                for dt in self.available_data_types()
                                if any(
                                    getattr(cd, dt, None) is not None
                                    for cd in climate_data[:10]
                                )
                            ],
                        },
                    )
            ...
            logger.error(
                f"Unexpected error exporting station {self.id} to repository: {str(e)}",
                exc_info=True,
            )
            return None, False
...
class WeatherDataType(models.Model):
    ...
    unit = models.CharField(
        max_length=50, blank=True, null=True, help_text="Unit of measurement"
    )
    min_value = models.FloatField(
        null=True, blank=True, help_text="Minimum valid value"
    )
    max_value = models.FloatField(
        null=True, blank=True, help_text="Maximum valid value"
    )
    icon = models.CharField(
        max_length=50, blank=True, null=True, help_text="Icon identifier for UI display"
    )
...
class ClimateData(models.Model):
    ...
    station = models.ForeignKey(
        WeatherStation, on_delete=models.CASCADE, related_name="climate_data"
    )
    timestamp = models.DateTimeField(db_index=True)
    year = models.IntegerField(
        db_index=True,
        help_text="Year of the measurement for easier filtering",
        default=2000,
    )
    month = models.IntegerField(
        db_index=True,
        help_text="Month of the measurement for easier filtering",
        default=1,
    )
    ...
    air_quality_index = models.IntegerField(
        validators=[MinValueValidator(0), MaxValueValidator(500)], null=True, blank=True
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["station", "timestamp"], name="unique_station_timestamp"
            )
        ]
...
class DataExport(models.Model):
    ...
    stations = models.ManyToManyField(
        WeatherStation, blank=True, related_name="exports"
    )
    country = models.ForeignKey(
    ...
    data_types = models.ManyToManyField(
        ...
        help_text="Types of weather data to include",
    )
    bounding_box = models.PolygonField(
        srid=4326, null=True, blank=True, help_text="Geographic area for the export"
    )
    ...
    years = ArrayField(
        ...
        blank=True,
        null=True,
        help_text="Specific years to include",
    )
    min_data_quality = models.CharField(
        max_length=10, choices=ClimateData.QUALITY_CHOICES, default="medium"
    )
    ...
    download_count = models.PositiveIntegerField(
        default=0, help_text="Number of times this export has been downloaded"
    )
    error_log = models.TextField(blank=True, null=True)

    # New fields we're adding
    file = models.FileField(
        upload_to="exports/", null=True, blank=True, help_text="The exported data file"
    )
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return (
            f"Export {self.id} - {self.get_export_format_display()} - {self.created_at}"
        )
    ...
    def get_download_url(self):
        ...
        return reverse(
            "maps:secure_export_download",
            kwargs={"export_id": self.id, "signature": signature},
        )
...
class WeatherAlert(models.Model):
    ...
    affected_area = models.PolygonField(
        ...
        null=True,
        blank=True,
        help_text="Geographic area affected by this alert",
    )
    ...
    data_type = models.ForeignKey(
        WeatherDataType, on_delete=models.CASCADE, related_name="alerts", null=True
    )
