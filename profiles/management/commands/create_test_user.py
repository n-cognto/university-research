...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--id_number", type=str, help="Custom ID number (default: random)"
        )
        parser.add_argument(
            "--password", type=str, default="testpassword", help="Password for the user"
        )
    ...
    def handle(self, *args, **options):
        ...
        if Profile.objects.filter(id_number=id_number).exists():
            self.stdout.write(
                self.style.ERROR(f"ID number {id_number} already exists!")
            )
            return
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    "You can now log in with this ID number and password"
                )
            )
